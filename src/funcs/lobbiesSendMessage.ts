 ...
 */
export function lobbiesSendMessage(
  client: DiscordCore,
  ...
  Result<
    operations.CreateLobbyMessageResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.CreateLobbyMessageRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.SDKMessageRequest, { explode: true });
  ...
  const path = pathToFunc("/lobbies/{lobby_id}/messages")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(201, operations.CreateLobbyMessageResponse$inboundSchema, {
      hdrs: true,
      key: "Result",
    }),
