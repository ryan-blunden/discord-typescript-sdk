 ...
 */
export function guildsCreateChannel(
  client: DiscordCore,
  request: operations.CreateGuildChannelRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.CreateGuildChannelResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.CreateGuildChannelRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.CreateGuildChannelRequest, {
    explode: true,
  });
  ...
  const path = pathToFunc("/guilds/{guild_id}/channels")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(201, operations.CreateGuildChannelResponse$inboundSchema, {
      hdrs: true,
      key: "Result",
    }),
