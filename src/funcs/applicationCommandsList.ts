 ...
 */
export function applicationCommandsList(
  client: DiscordCore,
  ...
  Result<
    operations.ListApplicationCommandsResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.ListApplicationCommandsRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/applications/{application_id}/commands")(
    pathParams,
  );

  const query = encodeFormQuery({
    "with_localizations": payload.with_localizations,
  });
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.ListApplicationCommandsResponse$inboundSchema, {
      hdrs: true,
      key: "Result",
    }),
