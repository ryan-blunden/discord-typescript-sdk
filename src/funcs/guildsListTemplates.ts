...
export async function guildsListTemplates(
  client: DiscordCore,
  request: operations.ListGuildTemplatesRequest,
  options?: RequestOptions,
): Promise<
  Result<
    Array<components.GuildTemplateResponse>,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.ListGuildTemplatesRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/guilds/{guild_id}/templates")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, z.array(components.GuildTemplateResponse$inboundSchema)),
