 ...
 */
export function pollsExpire(
  client: DiscordCore,
  request: operations.PollExpireRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.PollExpireResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.PollExpireRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/channels/{channel_id}/polls/{message_id}/expire")(
    pathParams,
  );
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.PollExpireResponse$inboundSchema, {
      hdrs: true,
      key: "Result",
    }),
