...
export function guildSearch(
  ...
  Result<
    operations.GuildSearchResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.GuildSearchRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/guilds/{guild_id}/messages/search")(pathParams);

  const query = encodeFormQuery({
    "attachment_extension": payload.attachment_extension,
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.GuildSearchResponse$inboundSchema, {
      hdrs: true,
      key: "Result",
    }),
    M.json(202, operations.GuildSearchResponse$inboundSchema, {
      hdrs: true,
      key: "Result",
    }),
