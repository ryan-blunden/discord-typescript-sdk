 ...
 */
export function soundboardsUpdateGuildSoundboardSound(
  client: DiscordCore,
  request: operations.UpdateGuildSoundboardSoundRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.UpdateGuildSoundboardSoundResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.UpdateGuildSoundboardSoundRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.SoundboardPatchRequestPartial, {
    explode: true,
  });
  ...
  const pathParams = {
    ...
    sound_id: encodeSimple("sound_id", payload.sound_id, {
      explode: false,
      charEncoding: "percent",
    }),
  };

  const path = pathToFunc("/guilds/{guild_id}/soundboard-sounds/{sound_id}")(
    pathParams,
  );
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.UpdateGuildSoundboardSoundResponse$inboundSchema, {
      hdrs: true,
      key: "Result",
    }),
