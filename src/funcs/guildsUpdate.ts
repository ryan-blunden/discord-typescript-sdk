 ...
 */
export function guildsUpdate(
  client: DiscordCore,
  request: operations.UpdateGuildRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.UpdateGuildResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.UpdateGuildRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.GuildPatchRequestPartial, {
    explode: true,
  });
  ...
  const path = pathToFunc("/guilds/{guild_id}")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, operations.UpdateGuildResponse$inboundSchema, {
      hdrs: true,
      key: "Result",
    }),
