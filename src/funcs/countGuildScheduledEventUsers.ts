 ...
 */
export function countGuildScheduledEventUsers(
  client: DiscordCore,
  request: operations.CountGuildScheduledEventUsersRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.CountGuildScheduledEventUsersResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.CountGuildScheduledEventUsersRequest$outboundSchema.parse(
        value,
      ),
  ...
  const query = encodeFormQuery({
    "guild_scheduled_event_exception_ids":
      payload.guild_scheduled_event_exception_ids,
  });
  ...
  >(
    M.json(
      200,
      operations.CountGuildScheduledEventUsersResponse$inboundSchema,
