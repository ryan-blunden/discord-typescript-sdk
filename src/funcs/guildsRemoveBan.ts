 ...
 */
export function guildsRemoveBan(
  client: DiscordCore,
  request: operations.UnbanUserFromGuildRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.UnbanUserFromGuildResponse | undefined,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.UnbanUserFromGuildRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.UnbanUserFromGuildRequest, {
    explode: true,
  });
  ...
  const path = pathToFunc("/guilds/{guild_id}/bans/{user_id}")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.nil(204, operations.UnbanUserFromGuildResponse$inboundSchema.optional(), {
      hdrs: true,
    }),
