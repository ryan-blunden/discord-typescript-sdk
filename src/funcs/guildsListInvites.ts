...
export async function guildsListInvites(
  client: DiscordCore,
  request: operations.ListGuildInvitesRequest,
  options?: RequestOptions,
): Promise<
  Result<
    Array<operations.ListGuildInvitesResponseBody>,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.ListGuildInvitesRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/guilds/{guild_id}/invites")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, z.array(operations.ListGuildInvitesResponseBody$inboundSchema)),
