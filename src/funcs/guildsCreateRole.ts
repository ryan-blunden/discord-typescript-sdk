 ...
 */
export function guildsCreateRole(
  client: DiscordCore,
  request: operations.CreateGuildRoleRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.CreateGuildRoleResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.CreateGuildRoleRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.CreateRoleRequest, { explode: true });
  ...
  const path = pathToFunc("/guilds/{guild_id}/roles")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.CreateGuildRoleResponse$inboundSchema, {
      hdrs: true,
      key: "Result",
    }),
