...
export async function guildsAutoModerationRulesUpdate(
  client: DiscordCore,
  request: operations.UpdateAutoModerationRuleRequest,
  options?: RequestOptions,
): Promise<
  Result<
    operations.UpdateAutoModerationRuleResponseBody,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.UpdateAutoModerationRuleRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/guilds/{guild_id}/auto-moderation/rules/{rule_id}")(
    pathParams,
  );
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, operations.UpdateAutoModerationRuleResponseBody$inboundSchema),
