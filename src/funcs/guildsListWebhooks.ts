...
export async function guildsListWebhooks(
  client: DiscordCore,
  request: operations.GetGuildWebhooksRequest,
  options?: RequestOptions,
): Promise<
  Result<
    Array<operations.GetGuildWebhooksResponseBody>,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.GetGuildWebhooksRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/guilds/{guild_id}/webhooks")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, z.array(operations.GetGuildWebhooksResponseBody$inboundSchema)),
