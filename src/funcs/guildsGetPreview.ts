...
export async function guildsGetPreview(
  client: DiscordCore,
  request: operations.GetGuildPreviewRequest,
  options?: RequestOptions,
): Promise<
  Result<
    components.GuildPreviewResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.GetGuildPreviewRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/guilds/{guild_id}/preview")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.GuildPreviewResponse$inboundSchema),
