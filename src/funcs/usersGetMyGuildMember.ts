...
export async function usersGetMyGuildMember(
  client: DiscordCore,
  request: operations.GetMyGuildMemberRequest,
  options?: RequestOptions,
): Promise<
  Result<
    components.PrivateGuildMemberResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.GetMyGuildMemberRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/users/@me/guilds/{guild_id}/member")(pathParams);
  ...
  const context = {
    ...
    oAuth2Scopes: ["guilds.members.read"],
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.PrivateGuildMemberResponse$inboundSchema),
