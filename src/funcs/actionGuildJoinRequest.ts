 ...
 */
export function actionGuildJoinRequest(
  client: DiscordCore,
  request: operations.ActionGuildJoinRequestRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.ActionGuildJoinRequestResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.ActionGuildJoinRequestRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/guilds/{guild_id}/requests/{request_id}")(
    pathParams,
  );
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.ActionGuildJoinRequestResponse$inboundSchema, {
      hdrs: true,
      key: "Result",
    }),
