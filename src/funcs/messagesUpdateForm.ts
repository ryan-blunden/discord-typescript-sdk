 ...
 */
export function messagesUpdateForm(
  client: DiscordCore,
  request: operations.UpdateMessageFormRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.UpdateMessageFormResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.UpdateMessageFormRequest$outboundSchema.parse(value),
  ...
  const body = Object.entries(payload.MessageEditRequestPartial || {}).map(
    ([k, v]) => {
  ...
  const context = {
    ...
    operationID: "update_message_form",
    oAuth2Scopes: null,
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.UpdateMessageFormResponse$inboundSchema, {
      hdrs: true,
      key: "Result",
    }),
