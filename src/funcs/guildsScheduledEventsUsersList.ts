...
export async function guildsScheduledEventsUsersList(
  client: DiscordCore,
  request: operations.ListGuildScheduledEventUsersRequest,
  options?: RequestOptions,
): Promise<
  Result<
    Array<components.ScheduledEventUserResponse>,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.ListGuildScheduledEventUsersRequest$outboundSchema.parse(
        value,
      ),
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, z.array(components.ScheduledEventUserResponse$inboundSchema)),
