...
export async function channelsGetAnswerVoters(
  client: DiscordCore,
  request: operations.GetAnswerVotersRequest,
  options?: RequestOptions,
): Promise<
  Result<
    components.PollAnswerDetailsResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.GetAnswerVotersRequest$outboundSchema.parse(value),
  ...
  const pathParams = {
    answer_id: encodeSimple("answer_id", payload.answer_id, {
      explode: false,
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.PollAnswerDetailsResponse$inboundSchema),
