...
export async function applicationsUpdateSelf(
  client: DiscordCore,
  request: components.ApplicationFormPartial,
  options?: RequestOptions,
): Promise<
  Result<
    components.PrivateApplicationResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) => components.ApplicationFormPartial$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/applications/@me")();
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.PrivateApplicationResponse$inboundSchema),
