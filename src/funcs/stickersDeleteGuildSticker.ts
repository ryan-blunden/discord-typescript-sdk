 ...
 */
export function stickersDeleteGuildSticker(
  client: DiscordCore,
  request: operations.DeleteGuildStickerRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.DeleteGuildStickerResponse | undefined,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.DeleteGuildStickerRequest$outboundSchema.parse(value),
  ...
  const pathParams = {
    ...
    sticker_id: encodeSimple("sticker_id", payload.sticker_id, {
      explode: false,
      charEncoding: "percent",
    }),
  };

  const path = pathToFunc("/guilds/{guild_id}/stickers/{sticker_id}")(
    pathParams,
  );
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.nil(204, operations.DeleteGuildStickerResponse$inboundSchema.optional(), {
      hdrs: true,
    }),
