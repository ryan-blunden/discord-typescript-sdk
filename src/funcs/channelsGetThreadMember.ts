...
export async function channelsGetThreadMember(
  client: DiscordCore,
  request: operations.GetThreadMemberRequest,
  options?: RequestOptions,
): Promise<
  Result<
    components.ThreadMemberResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.GetThreadMemberRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/channels/{channel_id}/thread-members/{user_id}")(
    pathParams,
  );
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.ThreadMemberResponse$inboundSchema),
