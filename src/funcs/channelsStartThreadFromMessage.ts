 ...
 */
export function channelsStartThreadFromMessage(
  client: DiscordCore,
  request: operations.CreateThreadFromMessageRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.CreateThreadFromMessageResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.CreateThreadFromMessageRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.CreateTextThreadWithMessageRequest, {
    explode: true,
  });
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(201, operations.CreateThreadFromMessageResponse$inboundSchema, {
      hdrs: true,
      key: "Result",
    }),
