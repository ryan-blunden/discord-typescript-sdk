 ...
 */
export function guildsBulkBan(
  client: DiscordCore,
  request: operations.BulkBanUsersFromGuildRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.BulkBanUsersFromGuildResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.BulkBanUsersFromGuildRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.BulkBanUsersRequest, {
    explode: true,
  });
  ...
  const path = pathToFunc("/guilds/{guild_id}/bulk-ban")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, operations.BulkBanUsersFromGuildResponse$inboundSchema, {
      hdrs: true,
      key: "Result",
    }),
