 ...
 */
export function voiceUpdateUserState(
  client: DiscordCore,
  request: operations.UpdateVoiceStateRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.UpdateVoiceStateResponse | undefined,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.UpdateVoiceStateRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.UpdateVoiceStateRequestPartial, {
    explode: true,
  });
  ...
  const path = pathToFunc("/guilds/{guild_id}/voice-states/{user_id}")(
    pathParams,
  );
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.nil(204, operations.UpdateVoiceStateResponse$inboundSchema.optional(), {
      hdrs: true,
    }),
