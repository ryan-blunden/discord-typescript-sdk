...
export function botPartnerSDKToken(
  client: DiscordCore,
  request: operations.BotPartnerSDKTokenRequestBody,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.BotPartnerSDKTokenResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.BotPartnerSDKTokenRequestBody$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/partner-sdk/token/bot")();
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.BotPartnerSDKTokenResponse$inboundSchema, {
      hdrs: true,
      key: "Result",
    }),
