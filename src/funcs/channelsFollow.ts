...
export async function channelsFollow(
  client: DiscordCore,
  request: operations.FollowChannelRequest,
  options?: RequestOptions,
): Promise<
  Result<
    components.ChannelFollowerResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.FollowChannelRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/channels/{channel_id}/followers")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.ChannelFollowerResponse$inboundSchema),
