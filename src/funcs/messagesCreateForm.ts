 ...
 */
export function messagesCreateForm(
  client: DiscordCore,
  request: operations.CreateMessageFormRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.CreateMessageFormResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.CreateMessageFormRequest$outboundSchema.parse(value),
  ...
  const body = Object.entries(payload.MessageCreateRequest || {}).map(
    ([k, v]) => {
  ...
  const context = {
    ...
    operationID: "create_message_form",
    oAuth2Scopes: null,
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.CreateMessageFormResponse$inboundSchema, {
      hdrs: true,
      key: "Result",
    }),
