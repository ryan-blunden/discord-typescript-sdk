...
export async function channelsThreadsArchivedPrivateList(
  client: DiscordCore,
  request: operations.ListPrivateArchivedThreadsRequest,
  options?: RequestOptions,
): Promise<
  Result<
    components.ThreadsResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.ListPrivateArchivedThreadsRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/channels/{channel_id}/threads/archived/private")(
    pathParams,
  );
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.ThreadsResponse$inboundSchema),
