 ...
 */
export function guildsSetMfaLevel(
  client: DiscordCore,
  request: operations.SetGuildMfaLevelRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.SetGuildMfaLevelResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.SetGuildMfaLevelRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.GuildMFARequest, { explode: true });
  ...
  const path = pathToFunc("/guilds/{guild_id}/mfa")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.SetGuildMfaLevelResponse$inboundSchema, {
      hdrs: true,
      key: "Result",
    }),
