...
export async function guildsSearchMembers(
  client: DiscordCore,
  request: operations.SearchGuildMembersRequest,
  options?: RequestOptions,
): Promise<
  Result<
    Array<components.GuildMemberResponse>,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.SearchGuildMembersRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/guilds/{guild_id}/members/search")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, z.array(components.GuildMemberResponse$inboundSchema)),
