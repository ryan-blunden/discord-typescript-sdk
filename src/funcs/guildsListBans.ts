...
export async function guildsListBans(
  client: DiscordCore,
  request: operations.ListGuildBansRequest,
  options?: RequestOptions,
): Promise<
  Result<
    Array<components.GuildBanResponse>,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.ListGuildBansRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/guilds/{guild_id}/bans")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, z.array(components.GuildBanResponse$inboundSchema)),
