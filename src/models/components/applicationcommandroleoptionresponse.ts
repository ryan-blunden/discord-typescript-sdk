/*
 * Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.
 */

import * as z from "zod/v3";
import { remap as remap$ } from "../../lib/primitives.js";
import { safeParse } from "../../lib/schemas.js";
import { ClosedEnum } from "../../types/enums.js";
import { Result as SafeParseResult } from "../../types/fp.js";
import { SDKValidationError } from "../errors/sdkvalidationerror.js";

/**
 * A sub-action within a command or group
 */
export const ApplicationCommandRoleOptionResponseType = {
  One: 1,
  Two: 2,
  Three: 3,
  Four: 4,
  Five: 5,
  Six: 6,
  Seven: 7,
  Eight: 8,
  Nine: 9,
  Ten: 10,
  Eleven: 11,
} as const;
/**
 * A sub-action within a command or group
 */
export type ApplicationCommandRoleOptionResponseType = ClosedEnum<
  typeof ApplicationCommandRoleOptionResponseType
>;

export type ApplicationCommandRoleOptionResponse = {
  type: ApplicationCommandRoleOptionResponseType;
  name: string;
  nameLocalized?: string | undefined;
  nameLocalizations?: { [k: string]: string } | null | undefined;
  description: string;
  descriptionLocalized?: string | undefined;
  descriptionLocalizations?: { [k: string]: string } | null | undefined;
  required?: boolean | undefined;
};

/** @internal */
export const ApplicationCommandRoleOptionResponseType$inboundSchema:
  z.ZodNativeEnum<typeof ApplicationCommandRoleOptionResponseType> = z
    .nativeEnum(ApplicationCommandRoleOptionResponseType);
/** @internal */
export const ApplicationCommandRoleOptionResponseType$outboundSchema:
  z.ZodNativeEnum<typeof ApplicationCommandRoleOptionResponseType> =
    ApplicationCommandRoleOptionResponseType$inboundSchema;

/** @internal */
export const ApplicationCommandRoleOptionResponse$inboundSchema: z.ZodType<
  ApplicationCommandRoleOptionResponse,
  z.ZodTypeDef,
  unknown
> = z.object({
  type: ApplicationCommandRoleOptionResponseType$inboundSchema,
  name: z.string(),
  name_localized: z.string().optional(),
  name_localizations: z.nullable(z.record(z.string())).optional(),
  description: z.string(),
  description_localized: z.string().optional(),
  description_localizations: z.nullable(z.record(z.string())).optional(),
  required: z.boolean().optional(),
}).transform((v) => {
  return remap$(v, {
    "name_localized": "nameLocalized",
    "name_localizations": "nameLocalizations",
    "description_localized": "descriptionLocalized",
    "description_localizations": "descriptionLocalizations",
  });
});
/** @internal */
export type ApplicationCommandRoleOptionResponse$Outbound = {
  type: number;
  name: string;
  name_localized?: string | undefined;
  name_localizations?: { [k: string]: string } | null | undefined;
  description: string;
  description_localized?: string | undefined;
  description_localizations?: { [k: string]: string } | null | undefined;
  required?: boolean | undefined;
};

/** @internal */
export const ApplicationCommandRoleOptionResponse$outboundSchema: z.ZodType<
  ApplicationCommandRoleOptionResponse$Outbound,
  z.ZodTypeDef,
  ApplicationCommandRoleOptionResponse
> = z.object({
  type: ApplicationCommandRoleOptionResponseType$outboundSchema,
  name: z.string(),
  nameLocalized: z.string().optional(),
  nameLocalizations: z.nullable(z.record(z.string())).optional(),
  description: z.string(),
  descriptionLocalized: z.string().optional(),
  descriptionLocalizations: z.nullable(z.record(z.string())).optional(),
  required: z.boolean().optional(),
}).transform((v) => {
  return remap$(v, {
    nameLocalized: "name_localized",
    nameLocalizations: "name_localizations",
    descriptionLocalized: "description_localized",
    descriptionLocalizations: "description_localizations",
  });
});

export function applicationCommandRoleOptionResponseToJSON(
  applicationCommandRoleOptionResponse: ApplicationCommandRoleOptionResponse,
): string {
  return JSON.stringify(
    ApplicationCommandRoleOptionResponse$outboundSchema.parse(
      applicationCommandRoleOptionResponse,
    ),
  );
}
export function applicationCommandRoleOptionResponseFromJSON(
  jsonString: string,
): SafeParseResult<ApplicationCommandRoleOptionResponse, SDKValidationError> {
  return safeParse(
    jsonString,
    (x) =>
      ApplicationCommandRoleOptionResponse$inboundSchema.parse(JSON.parse(x)),
    `Failed to parse 'ApplicationCommandRoleOptionResponse' from JSON`,
  );
}
