/*
 * Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.
 */

import { usersGetApplicationRoleConnection } from "../../funcs/usersGetApplicationRoleConnection.js";
import * as operations from "../../models/operations/index.js";
import { formatResult, ToolDefinition } from "../tools.js";

const args = {
  request: operations.GetApplicationUserRoleConnectionRequest$inboundSchema,
};

export const tool$usersGetApplicationRoleConnection: ToolDefinition<
  typeof args
> = {
  name: "users-get-application-role-connection",
  description:
    `Returns the application role connection for the user. Requires an OAuth2 access token with role_connections.write scope for the application specified in the path.`,
  scopes: ["users", "read"],
  args,
  tool: async (client, args, ctx) => {
    const [result, apiCall] = await usersGetApplicationRoleConnection(
      client,
      args.request,
      { fetchOptions: { signal: ctx.signal } },
    ).$inspect();

    if (!result.ok) {
      return {
        content: [{ type: "text", text: result.error.message }],
        isError: true,
      };
    }

    const value = result.value;

    return formatResult(value, apiCall);
  },
};
