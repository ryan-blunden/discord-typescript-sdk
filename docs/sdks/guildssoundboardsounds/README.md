...
async function run() {
  const result = await discord.guildsSoundboardSounds.list({
    guildId: "<value>",
  });
...
import { guildsSoundboardSoundsList } from "@ryan.blunden/discord-sdk/funcs/guildsSoundboardSoundsList.js";
...
async function run() {
  const res = await guildsSoundboardSoundsList(discord, {
    guildId: "<value>",
  });
