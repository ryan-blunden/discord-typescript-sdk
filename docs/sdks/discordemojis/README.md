...
async function run() {
  const result = await discord.guilds.emojis.list({
    guildId: "<value>",
  });
...
import { guildsEmojisList } from "@ryan.blunden/discord-sdk/funcs/guildsEmojisList.js";
...
async function run() {
  const res = await guildsEmojisList(discord, {
    guildId: "<value>",
  });
