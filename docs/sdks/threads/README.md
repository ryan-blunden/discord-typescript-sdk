...
* [listPublicArchived](#listpublicarchived)
...
async function run() {
  const result = await discord.threads.listPublicArchived({
    channelId: "<value>",
  });
...
import { threadsListPublicArchived } from "@ryan.blunden/discord-sdk/funcs/threadsListPublicArchived.js";
...
async function run() {
  const res = await threadsListPublicArchived(discord, {
    channelId: "<value>",
  });
