...
async function run() {
  const result = await discord.guilds.roles.update({
    guildId: "<value>",
    roleId: "<value>",
    requestBody: {},
  });
...
import { guildsRolesUpdate } from "@ryan.blunden/discord-sdk/funcs/guildsRolesUpdate.js";
...
async function run() {
  const res = await guildsRolesUpdate(discord, {
    guildId: "<value>",
    roleId: "<value>",
    requestBody: {},
  });
