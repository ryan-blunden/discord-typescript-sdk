...
async function run() {
  const result = await discord.guilds.mfa.setLevel({
    guildId: "<value>",
    requestBody: {},
  });
...
import { guildsMfaSetLevel } from "@ryan.blunden/discord-sdk/funcs/guildsMfaSetLevel.js";
...
async function run() {
  const res = await guildsMfaSetLevel(discord, {
    guildId: "<value>",
    requestBody: {},
  });
