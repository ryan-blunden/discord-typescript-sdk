...
async function run() {
  const result = await discord.guilds.integrations.list({
    guildId: "<value>",
  });
...
import { guildsIntegrationsList } from "@ryan.blunden/discord-sdk/funcs/guildsIntegrationsList.js";
...
async function run() {
  const res = await guildsIntegrationsList(discord, {
    guildId: "<value>",
  });
