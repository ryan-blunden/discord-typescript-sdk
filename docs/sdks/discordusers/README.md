...
async function run() {
  const result = await discord.guilds.scheduledEvents.users.list({
    guildId: "<value>",
...
import { guildsScheduledEventsUsersList } from "@ryan.blunden/discord-sdk/funcs/guildsScheduledEventsUsersList.js";
...
async function run() {
  const res = await guildsScheduledEventsUsersList(discord, {
    guildId: "<value>",
    guildScheduledEventId: "<value>",
  });
