...
async function run() {
  const result = await discord.users.getMyGuildMember({
    guildId: "<value>",
  });
...
```typescript
import { DiscordCore } from "@ryan.blunden/discord-sdk/core.js";
import { usersGetMyGuildMember } from "@ryan.blunden/discord-sdk/funcs/usersGetMyGuildMember.js";
...
async function run() {
  const res = await usersGetMyGuildMember(discord, {
    guildId: "<value>",
  });
