...
async function run() {
  const result = await discord.guildScheduledEventUsers.list({
    guildId: "<value>",
...
import { guildScheduledEventUsersList } from "@ryan.blunden/discord-sdk/funcs/guildScheduledEventUsersList.js";
...
async function run() {
  const res = await guildScheduledEventUsersList(discord, {
    guildId: "<value>",
    guildScheduledEventId: "<value>",
  });
