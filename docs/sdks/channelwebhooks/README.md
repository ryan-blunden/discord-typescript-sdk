...
async function run() {
  const result = await discord.channelWebhooks.list({
    channelId: "<value>",
  });
...
import { channelWebhooksList } from "@ryan.blunden/discord-sdk/funcs/channelWebhooksList.js";
...
async function run() {
  const res = await channelWebhooksList(discord, {
    channelId: "<value>",
  });
