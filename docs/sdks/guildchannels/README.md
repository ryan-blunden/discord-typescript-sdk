...
async function run() {
  const result = await discord.guildChannels.create({
    guildId: "<value>",
...
import { guildChannelsCreate } from "@ryan.blunden/discord-sdk/funcs/guildChannelsCreate.js";
...
async function run() {
  const res = await guildChannelsCreate(discord, {
    guildId: "<value>",
    createGuildChannelRequest: {
      name: "<value>",
    },
