...
async function run() {
  const result = await discord.guildRoles.create({
    guildId: "<value>",
    requestBody: {},
  });
...
import { guildRolesCreate } from "@ryan.blunden/discord-sdk/funcs/guildRolesCreate.js";
...
async function run() {
  const res = await guildRolesCreate(discord, {
    guildId: "<value>",
    requestBody: {},
  });
