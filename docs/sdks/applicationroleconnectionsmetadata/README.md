...
async function run() {
  const result = await discord.applicationRoleConnectionsMetadata.get({
    applicationId: "<value>",
  });
...
import { applicationRoleConnectionsMetadataGet } from "@ryan.blunden/discord-sdk/funcs/applicationRoleConnectionsMetadataGet.js";
...
async function run() {
  const res = await applicationRoleConnectionsMetadataGet(discord, {
    applicationId: "<value>",
  });
