...
async function run() {
  const result = await discord.guildIntegrations.list({
    guildId: "<value>",
  });
...
import { guildIntegrationsList } from "@ryan.blunden/discord-sdk/funcs/guildIntegrationsList.js";
...
async function run() {
  const res = await guildIntegrationsList(discord, {
    guildId: "<value>",
  });
