...
async function run() {
  const result = await discord.gateway.getBot();
...
import { gatewayGetBot } from "@ryan.blunden/discord-sdk/funcs/gatewayGetBot.js";
...
async function run() {
  const res = await gatewayGetBot(discord);
...
async function run() {
  const result = await discord.gateway.get();
...
```typescript
import { DiscordCore } from "@ryan.blunden/discord-sdk/core.js";
import { gatewayGet } from "@ryan.blunden/discord-sdk/funcs/gatewayGet.js";
...
async function run() {
  const res = await gatewayGet(discord);
