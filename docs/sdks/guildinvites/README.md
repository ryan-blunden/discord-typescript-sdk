...
async function run() {
  const result = await discord.guildInvites.list({
    guildId: "<value>",
  });
...
import { guildInvitesList } from "@ryan.blunden/discord-sdk/funcs/guildInvitesList.js";
...
async function run() {
  const res = await guildInvitesList(discord, {
    guildId: "<value>",
  });
