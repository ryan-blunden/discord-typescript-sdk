...
async function run() {
  const result = await discord.guilds.regions.list({
    guildId: "<value>",
  });
...
import { guildsRegionsList } from "@ryan.blunden/discord-sdk/funcs/guildsRegionsList.js";
...
async function run() {
  const res = await guildsRegionsList(discord, {
    guildId: "<value>",
  });
