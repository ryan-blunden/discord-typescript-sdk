...
async function run() {
  const result = await discord.stickerPacks.list();
...
import { stickerPacksList } from "@ryan.blunden/discord-sdk/funcs/stickerPacksList.js";
...
async function run() {
  const res = await stickerPacksList(discord);
...
async function run() {
  const result = await discord.stickerPacks.get({
    packId: "<value>",
  });
...
```typescript
import { DiscordCore } from "@ryan.blunden/discord-sdk/core.js";
import { stickerPacksGet } from "@ryan.blunden/discord-sdk/funcs/stickerPacksGet.js";
...
async function run() {
  const res = await stickerPacksGet(discord, {
    packId: "<value>",
  });
