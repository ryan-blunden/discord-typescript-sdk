...
async function run() {
  const result = await discord.guildThreads.getActive({
    guildId: "<value>",
  });
...
import { guildThreadsGetActive } from "@ryan.blunden/discord-sdk/funcs/guildThreadsGetActive.js";
...
async function run() {
  const res = await guildThreadsGetActive(discord, {
    guildId: "<value>",
  });
