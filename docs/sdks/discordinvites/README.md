...
async function run() {
  const result = await discord.channels.invites.list({
    channelId: "<value>",
  });
...
import { channelsInvitesList } from "@ryan.blunden/discord-sdk/funcs/channelsInvitesList.js";
...
async function run() {
  const res = await channelsInvitesList(discord, {
    channelId: "<value>",
  });
