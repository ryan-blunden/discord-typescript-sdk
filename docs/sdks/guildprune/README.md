...
async function run() {
  const result = await discord.guildPrune.preview({
    guildId: "<value>",
  });
...
import { guildPrunePreview } from "@ryan.blunden/discord-sdk/funcs/guildPrunePreview.js";
...
async function run() {
  const res = await guildPrunePreview(discord, {
    guildId: "<value>",
  });
