...
async function run() {
  const result = await discord.channels.threads.archived.private.list({
    channelId: "<value>",
  });
...
import { channelsThreadsArchivedPrivateList } from "@ryan.blunden/discord-sdk/funcs/channelsThreadsArchivedPrivateList.js";
...
async function run() {
  const res = await channelsThreadsArchivedPrivateList(discord, {
    channelId: "<value>",
  });
