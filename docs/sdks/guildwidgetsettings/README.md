...
async function run() {
  const result = await discord.guildWidgetSettings.update({
    guildId: "<value>",
    requestBody: {},
  });
...
import { guildWidgetSettingsUpdate } from "@ryan.blunden/discord-sdk/funcs/guildWidgetSettingsUpdate.js";
...
async function run() {
  const res = await guildWidgetSettingsUpdate(discord, {
    guildId: "<value>",
    requestBody: {},
  });
