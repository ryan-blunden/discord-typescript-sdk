...
async function run() {
  const result = await discord.channelPins.list({
    channelId: "<value>",
  });
...
import { channelPinsList } from "@ryan.blunden/discord-sdk/funcs/channelPinsList.js";
...
async function run() {
  const res = await channelPinsList(discord, {
    channelId: "<value>",
  });
