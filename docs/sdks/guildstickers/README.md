...
async function run() {
  const result = await discord.guildStickers.list({
    guildId: "<value>",
  });
...
import { guildStickersList } from "@ryan.blunden/discord-sdk/funcs/guildStickersList.js";
...
async function run() {
  const res = await guildStickersList(discord, {
    guildId: "<value>",
  });
