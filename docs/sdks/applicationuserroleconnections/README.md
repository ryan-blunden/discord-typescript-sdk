...
async function run() {
  const result = await discord.applicationUserRoleConnections.get({
    applicationId: "<value>",
  });
...
import { applicationUserRoleConnectionsGet } from "@ryan.blunden/discord-sdk/funcs/applicationUserRoleConnectionsGet.js";
...
async function run() {
  const res = await applicationUserRoleConnectionsGet(discord, {
    applicationId: "<value>",
  });
