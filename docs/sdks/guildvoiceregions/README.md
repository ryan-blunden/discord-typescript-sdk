...
async function run() {
  const result = await discord.guildVoiceRegions.list({
    guildId: "<value>",
  });
...
import { guildVoiceRegionsList } from "@ryan.blunden/discord-sdk/funcs/guildVoiceRegionsList.js";
...
async function run() {
  const res = await guildVoiceRegionsList(discord, {
    guildId: "<value>",
  });
